...
class WalytisBetaNetApi(BaseWalytisBetaInterface):
    ...
    @classmethod
    def join_blockchain(
        cls, invitation: str, blockchain_name: str = ""
    ) -> str:
    ...
    def create_invitation(
        cls, blockchain_id: str, one_time: bool = True, shared: bool = False
    ) -> str:
    ...
    @classmethod
    def get_walytis_beta_api_version(
        cls,
    ) -> tuple[int, int, int]:
    ...
    def get_and_read_block(cls, short_id: bytearray) -> Block:
        ...
        while short_id[-1] == 0:
            short_id = short_id[:-1]
        if bytearray(block.short_id) != short_from_long_id(
            short_id
        ):  # suspicious
            error_message = (
    ...
    def _read_appcomterm_error_message(
        cls, data: dict | bytearray, default_error: BaseException | None = None
    ) -> BaseException:
    ...
    @classmethod
    def _send_request(
        cls, function_name: str, payload: bytearray | bytes
    ) -> bytearray:
    ...
    @classmethod
    def _get_block_long_from_short_id(
        cls, blockchain_id: str, short_id: bytearray
    ):
